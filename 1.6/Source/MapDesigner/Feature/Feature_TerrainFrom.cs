...
namespace MapDesigner.Feature
{
    ...
    */
     ...
     * 
     * 
    [HarmonyPatch(typeof(RimWorld.MapGenUtility), "TerrainFrom")]
    [HarmonyBefore("GeologicalLandforms.Main")]
    public static class Feature_TerrainFrom
    {
        static bool Prefix(IntVec3 c, Map map, float elevation, float fertility, bool preferRock, ref TerrainDef __result)
        {
            if(fertility >= -1000)
            {
                return true;
