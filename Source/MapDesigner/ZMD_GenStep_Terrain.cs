...
{
    public class ZMD_GenStep_Terrain : GenStep
    {
  ...
  //      private sealed class <>c
		//{
		//	public static readonly GenStep_Terrain.<>c<>9 = new GenStep_Terrain.<>c();

  //      public static Func<Tile.RiverLink, int> <>9__5_0;

		//	public static Func<Tile.RiverLink, int> <>9__5_1;

		//	public static Func<RiverDef, float> <>9__8_0;

		//	internal int <GenerateRiver>b__5_0(Tile.RiverLink rl)
  //      {
  //          return -rl.river.degradeThreshold;
  //      }

  //      internal int <GenerateRiver>b__5_1(Tile.RiverLink rl)
  //      {
  //          return -rl.river.degradeThreshold;
  //      }

  //      internal float <GenerateRiverLookupTexture>b__8_0(RiverDef rd)
  //      {
  //          return rd.widthOnMap / 2f + 8f;
  //      }
  //  }
    ...
    {
        BeachMaker.Init(map);
        ZMD_RiverMaker riverMaker = this.GenerateRiver(map);
        ...
        BeachMaker.Cleanup();
        using (List<TerrainPatchMaker>.Enumerator enumerator2 = map.Biome.terrainPatchMakers.GetEnumerator())
        {
            while (enumerator2.MoveNext())
            {
                enumerator2.Current.Cleanup();
            }
        }
    }

    private TerrainDef TerrainFrom(IntVec3 c, Map map, float elevation, float fertility, ZMD_RiverMaker river, bool preferSolid)
    {
        ...
        if (!ZMD_GenStep_Terrain.debug_WarnedMissingTerrain)
        {
            Log.Error(string.Concat(new object[]
            {
                ...
                ZMD_GenStep_Terrain.debug_WarnedMissingTerrain = true;
        }
        return TerrainDefOf.Sand;
    }

    private ZMD_RiverMaker GenerateRiver(Map map)
    {
        ...
        if (rivers == null || rivers.Count == 0)
        {
            ...
            //Log.Message("The angle is " + angle);
            //angle = 150f;
            ...
            ZMD_RiverMaker riverMaker = new ZMD_RiverMaker(center, angle, (from rl in rivers
                                                                   orderby -rl.river.degradeThreshold
    ...
    private void UpdateRiverAnchorEntry(Dictionary<int, ZMD_GenStep_Terrain.GRLT_Entry> entries, IntVec3 center, int entryId, float zValue)
    {
        ...
        {
            entries[entryId] = new ZMD_GenStep_Terrain.GRLT_Entry
            {
                bestDistance = num,
                bestNode = center
            };
        ...
        private void GenerateRiverLookupTexture(Map map, ZMD_RiverMaker riverMaker)
        {
            int num = Mathf.CeilToInt((from rd in DefDatabase<RiverDef>.AllDefs
                                       select rd.widthOnMap / 2f + 8f).Max());
            int num2 = Mathf.Max(4, num) * 2;
            Dictionary<int, ZMD_GenStep_Terrain.GRLT_Entry> dictionary = new Dictionary<int, ZMD_GenStep_Terrain.GRLT_Entry>();
            Dictionary<int, ZMD_GenStep_Terrain.GRLT_Entry> dictionary2 = new Dictionary<int, ZMD_GenStep_Terrain.GRLT_Entry>();
            Dictionary<int, ZMD_GenStep_Terrain.GRLT_Entry> dictionary3 = new Dictionary<int, ZMD_GenStep_Terrain.GRLT_Entry>();
